...
# This deployment uses an external BGE-M3 embedding service and an external
# bge-reranker-v2-m3 service. Only the small BGE-M3 tokenizer asset is baked
# into RAGFlow for accurate local token counting; no embedding/reranker model
# weights are copied into this image and no GPU memory is consumed by this file.
RUN --mount=type=bind,from=infiniflow/ragflow_deps:latest,source=/huggingface.co,target=/huggingface.co \
    asset="BAAI/bge-m3/sentencepiece.bpe.model"; \
    if [ -f "/huggingface.co/$asset" ]; then \
        ...
        echo "ERROR: tokenizer asset $asset is missing from the infiniflow/ragflow_deps image" >&2; \
        exit 1; \
    fi
